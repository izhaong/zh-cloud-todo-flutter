...
- **不维护 `scripts/` 目录**：构建与打包逻辑在 **`.github/workflows/desktop.yml`** 与 **`packaging/`** 配置中。
- **Release 安装包对齐 [RustDesk Releases](https://github.com/rustdesk/rustdesk/releases)**：原生安装器，而非 zip/tar.gz 压缩包。
...
与 `desktop.yml` 中单平台步骤相同，例如 macOS DMG：
...
hdiutil create -volname "Todo Flutter" -ov -format UDZO \
  ...
  todo-flutter-1.0.0-aarch64.dmg
```
